...
class WatertankChartView @JvmOverloads constructor(
    context: Context,
    ...
    private var records: List<WaterTankRecord> = emptyList()
    ...
    // Bar dimensions (larger bars like Swift)
    private val barWidth = 18f
    private val barSpacing = 6f
    
    fun updateData(records: List<WaterTankRecord>) {
    ...
    private fun drawData(canvas: Canvas) {
        ...
        allDates.forEachIndexed { index, date ->
            ...
            val water = record?.newWater?.toDoubleOrNull() ?: 0.0
            val normalizedValue = if (maxValue > 0) water / maxValue else 0.0
            ...
            if (barLeft + barWidth >= chartLeft && barLeft <= chartRight) {
                if (water > 0) {
                    ...
                    if (barHeight > 20 && record != null) {
                        ...
                        val valueText = formatValue(water)
                        canvas.drawText(
                            valueText,
                            barLeft + (barWidth / 2),
                            barTop - 8,
    ...
    private fun drawLabels(canvas: Canvas) {
        ...
        // Draw unit label
        textPaint.textSize = 20f
        canvas.drawText("m³", chartLeft - 10f, chartTop - 10f, textPaint)
    ...
    private fun getMaxWaterValue(): Double {
        if (records.isEmpty()) return 100.0
        
        val maxWater = records.maxOfOrNull { record ->
            record.newWater.toDoubleOrNull() ?: 0.0
        } ?: 100.0
        
        return if (maxWater == 0.0) 100.0 else maxWater
    }
