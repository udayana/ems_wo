...
class PdamChartView @JvmOverloads constructor(
    context: Context,
    ...
    private var records: List<PDAMRecord> = emptyList()
    ...
    fun setData(records: List<PDAMRecord>) {
    ...
    private fun drawData(canvas: Canvas) {
        ...
        allDates.forEachIndexed { index, date ->
            ...
            val normalizedValue = if (maxValue > 0) consume / maxValue else 0.0
            ...
            if (barLeft + barWidth >= chartLeft && barLeft <= chartRight) {
                if (consume > 0) {
                    ...
                    if (barHeight > 20 && record != null) {
                        ...
                        val valueText = formatValue(consume)
                        canvas.drawText(
                            valueText,
                            barLeft + (barWidth / 2),
                            barTop - 8,
    ...
    private fun getMaxValue(): Double {
        ...
        val maxConsume = records.maxOfOrNull { record ->
            record.consume.toDoubleOrNull() ?: 0.0
        } ?: 100.0
        
        return if (maxConsume == 0.0) 100.0 else maxConsume
    }
