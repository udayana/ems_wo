...
class HeatPumpChartView @JvmOverloads constructor(
    context: Context,
    ...
    private var records: List<HeatPumpRecord> = emptyList()
    ...
    fun setData(records: List<HeatPumpRecord>) {
    ...
    private fun drawData(canvas: Canvas) {
        ...
        allDates.forEachIndexed { index, date ->
            ...
            val amp = record?.amp?.toDoubleOrNull() ?: 0.0
            val normalizedValue = if (maxValue > 0) amp / maxValue else 0.0
    ...
    private fun drawLabels(canvas: Canvas) {
        ...
        // Draw unit label
        textPaint.textSize = 20f
        canvas.drawText("Amp", chartLeft - 10f, chartTop - 10f, textPaint)
    ...
    private fun getMaxAmpValue(): Double {
        if (records.isEmpty()) return 100.0
        
        val maxAmp = records.maxOfOrNull { record ->
            record.amp.toDoubleOrNull() ?: 0.0
        } ?: 100.0
        
        return if (maxAmp == 0.0) 100.0 else maxAmp
    }
