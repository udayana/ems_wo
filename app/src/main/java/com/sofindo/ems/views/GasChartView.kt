...
class GasChartView @JvmOverloads constructor(
    context: Context,
    ...
    private var records: List<GasRecord> = emptyList()
    ...
    fun updateData(records: List<GasRecord>) {
    ...
    private fun drawData(canvas: Canvas) {
        ...
        allDates.forEachIndexed { index, date ->
            ...
            val gas = record?.newGas?.toDoubleOrNull() ?: 0.0
            val normalizedValue = if (maxValue > 0) gas / maxValue else 0.0
    ...
    private fun drawLabels(canvas: Canvas) {
        ...
        // Draw unit label
        textPaint.textSize = 20f
        canvas.drawText("tabung", chartLeft - 10f, chartTop - 10f, textPaint)
    ...
    private fun getMaxGasValue(): Double {
        if (records.isEmpty()) return 100.0
        
        val maxGas = records.maxOfOrNull { record ->
            record.newGas.toDoubleOrNull() ?: 0.0
        } ?: 100.0
        
        return if (maxGas == 0.0) 100.0 else maxGas
    }
