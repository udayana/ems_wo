    ...
    private fun handleImageSelection(uri: Uri) {
        lifecycleScope.launch {
            try {
                ...
                selectedImageFile = withContext(Dispatchers.IO) {
                    resizeCrop(tempFile, size = 480, quality = 90)
                }
                
                // Show preview
                Glide.with(this@ChangePendingDoneActivity)
                ...
                Toast.makeText(this@ChangePendingDoneActivity, "Error loading image: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
