...
class ABTRecordAdapter(
    private val records: List<ABTRecord>,
    private val onDeleteClick: (ABTRecord) -> Unit
) : RecyclerView.Adapter<ABTRecordAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_abt_record, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // Format meter record
        holder.meterText.text = if (record.meterRecord.isEmpty() || record.meterRecord == "0") {
            "-"
        } else {
            record.meterRecord
        }
        ...
        // Format consume
        holder.consumeText.text = if (record.consume.isEmpty() || record.consume == "0") {
            "-"
        } else {
            "${record.consume} m3"
        }
        ...
        // Format cost
        holder.costText.text = if (record.estimateCost.isEmpty() || record.estimateCost == "0") {
            "-"
        } else {
            formatNumber(record.estimateCost.toDoubleOrNull() ?: 0.0)
        }
        
        // Format rec by
        holder.recByText.text = if (record.recBy.isEmpty()) {
            "-"
        } else {
            record.recBy
        }
        
        // Set double click listener for delete (only method)
